...
<head>
    ...
    <style>
        ...
        * {
        ...
        .btn-mapper.active-shift {
            background-color: var(--accent-green);
            color: #fff;
            box-shadow: 0 0 8px rgba(16, 185, 129, 0.4);
        }

        .btn-mapper.active-gear {
            background-color: var(--accent-cyan);
            color: #fff;
            box-shadow: 0 0 8px rgba(6, 182, 212, 0.4);
        }

        .btn-mapper.active-off {
            background-color: #334155;
            color: var(--text-primary);
        }
...
<body>
            ...
            <div id="tab-led" class="tab-content">
                ...
                <form id="led-form" onsubmit="saveConfig(event, 'shiftLight')">
                    ...
                    <div class="settings-grid">
                        ...
                        <div class="form-group">
                            <label for="sl-mode">Display Mode</label>
                            <select id="sl-mode" onchange="toggleLedModeFields(); drawLedPreview();">
                                <option value="0">Linear Strip</option>
                                <option value="1">8x8 Matrix</option>
                            </select>
                        </div>

                        <div class="form-group" id="sl-numleds-group">
                            <label for="sl-numleds">Number of LEDs</label>
                            <input type="number" id="sl-numleds" min="1" max="64" onchange="drawLedPreview()">
                        </div>

                        <div class="form-group" id="sl-serp-group" style="display:none;">
                            <label for="sl-serp">Matrix Layout Style</label>
                            <select id="sl-serp">
                                <option value="false">Progressive Grid</option>
                                <option value="true">Serpentine Grid</option>
                            </select>
                        </div>
                    </div>

                    <div id="sl-matrix-mapper" class="dash-card" style="display:none; margin-top:1.5rem; margin-bottom:1.5rem; border:1px solid rgba(255,255,255,0.08); background:rgba(30,41,59,0.2);">
                        <div class="dash-card-title" style="margin-bottom:0.4rem; font-size:0.95rem;">Interactive Matrix Row Mapper</div>
                        <div style="font-size:0.8rem; color:var(--text-secondary); margin-bottom:1rem;">
                            Assign rows for Shift Lights (contiguous green/red zones), Gear Display (contiguous cyan zone), or turn them Off.
                        </div>
                        <div id="sl-matrix-warning" style="display:none; color:var(--accent-orange); font-size:0.85rem; margin-bottom:1rem; padding:0.5rem; background:rgba(249,115,22,0.1); border-radius:6px; border:1px solid rgba(249,115,22,0.2);">
                            ⚠️ Shift and Gear zones cannot overlap or have gaps.
                        </div>
                        <div style="display:flex; flex-direction:column; gap:0.5rem;" id="matrix-rows-container">
    ...
    <script>
        ...
        let matrixRows = [0, 1, 1, 1, 1, 1, 1, 1]; // 0 = Shift, 1 = Gear, 2 = Off
        ...
        function toggleLedModeFields() {
            const mode = parseInt(document.getElementById('sl-mode').value) || 0;
            const numLedsGroup = document.getElementById('sl-numleds-group');
            const serpGroup = document.getElementById('sl-serp-group');
            const matrixMapper = document.getElementById('sl-matrix-mapper');
            
            if (mode === 0) {
                numLedsGroup.style.display = 'block';
                serpGroup.style.display = 'none';
                matrixMapper.style.display = 'none';
            } else {
                numLedsGroup.style.display = 'none';
                serpGroup.style.display = 'block';
                matrixMapper.style.display = 'block';
                document.getElementById('sl-numleds').value = 64;
                renderRowMapper();
            }
        }

        function renderRowMapper() {
            ...
            for (let i = 0; i < 8; i++) {
                ...
                item.className = 'row-mapper-item';
                item.style.cssText = 'display:flex; justify-content:space-between; align-items:center; padding:0.4rem 0.75rem; background:rgba(255,255,255,0.02); border-radius:6px; border:1px solid rgba(255,255,255,0.05);';
                
                const label = document.createElement('span');
                label.textContent = 'Row ' + i;
                label.style.cssText = 'font-weight:500; font-size:0.85rem; color:var(--text-primary);';
                ...
                btnShift.className = 'btn-mapper' + (matrixRows[i] === 0 ? ' active-shift' : '');
                btnShift.textContent = 'Shift';
                btnShift.onclick = () => setRowType(i, 0);
                btnGroup.appendChild(btnShift);
                
                const btnGear = document.createElement('button');
                btnGear.type = 'button';
                btnGear.className = 'btn-mapper' + (matrixRows[i] === 1 ? ' active-gear' : '');
                btnGear.textContent = 'Gear';
                btnGear.onclick = () => setRowType(i, 1);
                btnGroup.appendChild(btnGear);
                
                const btnOff = document.createElement('button');
                btnOff.type = 'button';
                btnOff.className = 'btn-mapper' + (matrixRows[i] === 2 ? ' active-off' : '');
                btnOff.textContent = 'Off';
                btnOff.onclick = () => setRowType(i, 2);
            ...
            validateAndCalculateLayout();
        }

        function setRowType(rowIdx, type) {
            matrixRows[rowIdx] = type;
            renderRowMapper();
            drawLedPreview();
        }

        function validateAndCalculateLayout() {
            let shiftRows = [];
            let gearRows = [];
            for (let i = 0; i < 8; i++) {
                if (matrixRows[i] === 0) shiftRows.push(i);
                if (matrixRows[i] === 1) gearRows.push(i);
            }
            ...
            if (shiftRows.length > 0) {
                let minS = Math.min(...shiftRows);
                let maxS = Math.max(...shiftRows);
                if (maxS - minS + 1 !== shiftRows.length) {
                    valid = false;
                    msg = "Shift light rows must be contiguous.";
                }
            }
            
            if (gearRows.length > 0) {
                let minG = Math.min(...gearRows);
                let maxG = Math.max(...gearRows);
                if (maxG - minG + 1 !== gearRows.length) {
                    valid = false;
                    msg = "Gear display rows must be contiguous.";
                }
            }
            
            if (shiftRows.length > 0 && gearRows.length > 0) {
                let minS = Math.min(...shiftRows);
                let maxS = Math.max(...shiftRows);
                let minG = Math.min(...gearRows);
                let maxG = Math.max(...gearRows);
                
                if (minS <= maxG && maxS >= minG) {
                    valid = false;
                    msg = "Shift lights and Gear display zones cannot overlap.";
                }
            }
            
            const warningEl = document.getElementById('sl-matrix-warning');
            const submitBtn = document.querySelector('#led-form button[type="submit"]');
        ...
        function drawLedPreview() {
            ...
            if (mode === 0) {
                preview.className = 'led-preview-strip';
                ...
                preview.className = 'led-preview-matrix';
                ledCount = 64;
                preview.innerHTML = '';
                for (let i = 0; i < ledCount; i++) {
                    const node = document.createElement('div');
                    node.className = 'led-node-matrix';
                    node.id = 'preview-led-' + i;
                    preview.appendChild(node);
                }
                highlightMatrixPreviewRows();
                simulateLedPreview(0.5);
            }
        }

        function highlightMatrixPreviewRows() {
            for (let i = 0; i < 8; i++) {
                let type = matrixRows[i];
                let borderStyle = '1px solid rgba(255,255,255,0.05)';
                if (type === 0) borderStyle = '1px dashed rgba(16, 185, 129, 0.4)';
                else if (type === 1) borderStyle = '1px dashed rgba(6, 182, 212, 0.4)';
                
                for (let x = 0; x < 8; x++) {
                    let ledIdx = i * 8 + x;
                    let led = document.getElementById('preview-led-' + ledIdx);
                    if (led) {
                        led.style.border = borderStyle;
                    }
                }
            }
        }
        ...
        function drawVirtualGear(gear, startRow, rowCount, colorHex) {
            if (rowCount === 0) return;
            const bitmap = GEAR_BITMAPS[gear < 12 ? gear : 11];
            
            let bmpStartRow = 0;
            let physStartRow = startRow;
            let linesToDraw = rowCount;

            if (rowCount >= 7) {
                physStartRow = startRow + Math.floor((rowCount - 7) / 2);
                linesToDraw = 7;
                bmpStartRow = 0;
            } else {
                bmpStartRow = Math.floor((7 - rowCount) / 2);
                linesToDraw = rowCount;
                physStartRow = startRow;
            }

            for (let y = 0; y < linesToDraw; y++) {
                let rowByte = bitmap[bmpStartRow + y];
                let physicalY = physStartRow + y;
                if (physicalY < 0 || physicalY > 7) continue;

                for (let x = 0; x < 8; x++) {
                    let pixelOn = (rowByte & (0x80 >> x)) !== 0;
                    if (pixelOn) {
                        let idx = physicalY * 8 + x;
                        let led = document.getElementById('preview-led-' + idx);
                        if (led) {
                            led.style.backgroundColor = colorHex;
                            led.style.boxShadow = `0 0 10px ${colorHex}, inset 0 1px 2px rgba(255,255,255,0.4)`;
                        }
                    }
                }
            }
        ...
        function simulateLedPreview(percent = 0.5, liveGear = 9) {
            const mode = parseInt(document.getElementById('sl-mode').value) || 0;
            ...
            if (mode === 0) {
                ...
                let matrixStartRow = shiftRows.length > 0 ? Math.min(...shiftRows) : 0;
                let matrixRowCount = shiftRows.length;
                let gearStartRow = gearRows.length > 0 ? Math.min(...gearRows) : 0;
                let gearRowCount = gearRows.length;

                let totalShiftLeds = matrixRowCount * 8;
                let litCount = Math.round(percent * totalShiftLeds);

                let flashColor = 'rgb(244, 63, 94)';
                if (scheme === 1) flashColor = 'rgb(168, 85, 247)';
                else if (scheme === 2) flashColor = '#ffffff';
                
                if (percent >= 1.0) {
                    const isEven = Math.floor(Date.now() / 80) % 2 === 0;
                    if (isEven) {
                        for (let i = 0; i < totalShiftLeds; i++) {
                            let row = matrixStartRow + Math.floor(i / 8);
                            let col = i % 8;
                            let idx = row * 8 + col;
                            let led = document.getElementById('preview-led-' + idx);
                            if (led) {
                                led.style.backgroundColor = flashColor;
                                led.style.boxShadow = `0 0 10px ${flashColor}, inset 0 1px 2px rgba(255,255,255,0.4)`;
                            }
                        }
                        if (gearRowCount > 0) {
                            drawVirtualGear(liveGear, gearStartRow, gearRowCount, flashColor);
                        }
                    }
                    return;
                }
                ...
                for (let i = 0; i < totalShiftLeds; i++) {
                    if (i < litCount) {
                        ...
                        let led = document.getElementById('preview-led-' + idx);
                        if (led) {
                            let color = getLedColor(i, totalShiftLeds, scheme);
                            led.style.backgroundColor = color;
                            led.style.boxShadow = `0 0 10px ${color}, inset 0 1px 2px rgba(255,255,255,0.4)`;
                        }
                    }
                ...
                if (gearRowCount > 0) {
                    drawVirtualGear(liveGear, gearStartRow, gearRowCount, '#00aaff');
                }
            }
        }
        ...
        function populateUI(cfg) {
            ...
            document.getElementById('sl-mode').value = (cfg.shiftLight.ledMode !== undefined ? cfg.shiftLight.ledMode : 0).toString();
            document.getElementById('sl-serp').value = (cfg.shiftLight.matrixSerpentine !== undefined ? cfg.shiftLight.matrixSerpentine : false).toString();

            matrixRows = Array(8).fill(2);
            if (cfg.shiftLight.ledMode === 1) {
                const mStart = cfg.shiftLight.matrixStartRow !== undefined ? cfg.shiftLight.matrixStartRow : 0;
                const mRows = cfg.shiftLight.matrixRowCount !== undefined ? cfg.shiftLight.matrixRowCount : 1;
                const gStart = cfg.shiftLight.gearStartRow !== undefined ? cfg.shiftLight.gearStartRow : 1;
                const gRows = cfg.shiftLight.gearRowCount !== undefined ? cfg.shiftLight.gearRowCount : 7;
                
                for (let i = 0; i < mRows; i++) {
                    let r = mStart + i;
                    if (r >= 0 && r < 8) matrixRows[r] = 0;
                }
                for (let i = 0; i < gRows; i++) {
                    let r = gStart + i;
                    if (r >= 0 && r < 8) matrixRows[r] = 1;
                }
            } else {
                matrixRows[0] = 0;
                for (let r = 1; r < 8; r++) matrixRows[r] = 1;
            }
            ...
            const wifiCardTitle = document.querySelector('#wifi-form').previousElementSibling;

            if (isApModeActive) {
                if (nav) nav.style.display = 'none';
                document.querySelectorAll('.tab-content').forEach(el => el.classList.remove('active'));
                document.getElementById('tab-net').classList.add('active');
                document.getElementById('wifi-form').parentElement.style.margin = '0 auto';
                ...
                document.getElementById('wifi-form').parentElement.style.margin = '';
                document.getElementById('wifi-form').parentElement.style.maxWidth = '';
                showTab('tab-dash');
            }
        }
        ...
        function pollTelemetry() {
            ...
            fetch(`${deviceIp}/api/status`)
                .then(res => res.json())
                .then(data => {
                    if (data.isApMode !== undefined && data.isApMode !== isApModeActive) {
                        connectToDevice();
                        return;
                    }
                    ...
                    if (data.isUpdating) {
                        ...
                        let gearIdx = data.gear;
                        
                        if (currentPct < slStart) {
                            simulateLedPreview(0, gearIdx);
                        } else if (currentPct >= slEnd) {
                            simulateLedPreview(1.0, gearIdx);
                        } else {
                            let fillPct = (currentPct - slStart) / (slEnd - slStart);
                            simulateLedPreview(fillPct, gearIdx);
                        }
                    }
                })
                .catch(err => {
        ...
        function saveConfig(e, section) {
            ...
            if (section === 'shiftLight') {
                ...
                payload.shiftLight = {
                    ...
                    ledMode: parseInt(document.getElementById('sl-mode').value) || 0,
                    matrixSerpentine: document.getElementById('sl-serp').value === "true",
                    matrixStartRow: matrixStartRow,
                    matrixRowCount: matrixRowCount,
                    gearStartRow: gearStartRow,
                    gearRowCount: gearRowCount
                };
            ...
            .then(res => {
                if (res.ok) {
                    ...
                    if (section === 'wifi') {
                        if (confirm("WiFi settings saved. Would you like to reboot the device now to connect?")) {
                            deviceAction('reboot');
                        }
                    }
                } else {
